...
def readRisksFile(fileName: str | Path) -> dict[str, FuzzySet]:
    ...
    with open(fileName, "r") as file:
        for line in file:
            ...
            x = np.arange(xmin, xmax + 1)
            y = fuzz.trapmf(x, points)
            variables[risk_name[1]] = FuzzySet(
                risk_name[0], risk_name[1], x, y
            )  # Ajustado según la nueva definición
    return variables
...
def readApplicationsFile(fileName: str | Path) -> dict[str, Application]:
    ...
    with open(fileName, "r") as file:
        for line in file:
            ...
            app_id = elements[0]
            data = [
                (elements[i], int(elements[i + 1])) for i in range(1, len(elements), 2)
            ]
