...
def desborrosificacion(
    x: numpy.ndarray | list | tuple, y: numpy.ndarray | list | tuple
):
    return skf.centroid(x, y)


def imprimir_funcion(
    nombre: str, x: numpy.ndarray | list, y: numpy.ndarray | list, centroide: int
) -> None:
    plt.clf()
    plt.plot(x, y)
    plt.xlabel("X")
...
def main():
    ...
    for aplicacion in aplicaciones.values():
        ...
        imprimir_funcion(
            aplicacion.app_id, funcion_agregada["x"], funcion_agregada["y"], centroide
        )
